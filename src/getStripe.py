    ...
    def extract(self, mp, bgleft, bgright):
        def search_frame(idx):
            ...
            if framesize > 10:
                ...
                temp_res = self.StripeSearch(D, idx, start, end, M, mp, chr, framesize, start_array,
                                                       end_array)
                return(temp_res)
            else:
                return 0
        ...
        for chridx in range(len(self.chromnames)):
            ...
            #result = Parallel(n_jobs = self.core)(delayed(obj.extract)(mp, bgleft, bgright) for mp in maxpixel)
            '''
            with Bar(chr, max=nframes) as bar:
                for i in range(nframes):
                    ...
                    if framesize > 10:
                        ...
                        start_array = [start_array[s] for s in nonzero_idx]
                        end_array = [end_array[s] for s in nonzero_idx]
                        temp_res, mw1, mw2 = self.StripeSearch(D, i, start, end, M, mp, chr, framesize, start_array, end_array)
