...
class getStripe:
    ...
    def _nulldist2(self):
        ...
        tabadjacent = np.zeros(10000)
        tabcenter = np.zeros(10000)
        distvec = np.zeros(10000)

        samplesize = (self.chromsizes / np.sum(self.chromsizes)) * 10000
        samplesize = np.uint16(samplesize)
        dif = 10000 - np.sum(samplesize)
        notzero = np.where(samplesize != 0)
        chromnames2 = [self.chromnames[i] for i in notzero[0]]
        samplesize[0] += dif
        c = 0
        with Bar('Background contrast distribution', max=len(chromnames2)) as bar:
            for chr in chromnames2:
                ...
                normmat = normmat[0]
                del mat
                nrow = normmat.shape[0]
                st = int(nrow * 0.1)
                en = int(nrow * 0.9)
                matsum = np.sum(normmat, axis=1)
                zeroindex = np.where(matsum == 0)

                ss = samplesize[c]
                pool = [x for x in list(range(st, en + 1)) if x not in zeroindex[0].tolist()]
                randval_x = random.choices(pool, k=ss)
                #randval_y = random.choices(pool, k=ss)
                randval_y = [random.choice(list(range(x-400,x+400))) for x in randval_x]
                if chr == chromnames2[0]:
                    i_start = 0
                    i_end = samplesize[0]
                else:
                ...
                for i in range(i_start, i_end):
                    x = randval_x[L]
                    y = randval_y[L]
                    x_adjacent=random.choice((range(x-7,x-2), range(x+3,x+8)))
                    L += 1
                    tabadjacent[i] = np.mean(normmat[(y - 2):(y + 3), list(x_adjacent)])
                    tabcenter[i] = np.mean(normmat[(y - 2):(y + 3), (x - 2):(x + 3)])
                    distvec[i] = abs(x-y)
                bar.next()
            bg = np.subtract(tabcenter, tabadjacent)
            del normmat
        print('Elapsed time for background estimation: ' + str(
            np.round((time.time() - t_background_start) / 60, 3)) + ' min')
        return bg, distvec
    ...
    def extract(self, mp, bgleft, bgright):
        ...
        for chridx in range(len(self.chromnames)):
            ...
            with Bar(chr, max=nframes) as bar:
                for i in range(nframes):
                    ...
                    # D = cfm[np.ix_(list(range(start,end+1)),list(range(start,end+1)))]
                    # framesize = D.shape[0]
                    ...
                    if framesize > 10:
                        ...
                        temp_res, mw1, mw2 = self.StripeSearch(D, i, start, end, M, mp, chr, framesize, start_array,
                                                               end_array)
    ...
    def StripeSearch(self, submat, num, start, end, M, mp, chr, framesize, start_array, end_array):
        ...
        return result, max_width_dist1, max_width_dist2
